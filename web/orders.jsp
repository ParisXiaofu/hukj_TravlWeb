        ...
        $(function () {
            $.post(
                ...
                function (data) {
                    ...
                    var toto=0;
                    $(data).each(function () {
                        result+=" <li>"+this.line.lineID+"</li>" +
                            " <li>"+this.line.lineName+this.line.days+"日游</li> " +
                            "<li >￥<span id='total'>"+this.line.price+"</span></li> " +
                            "<li>"+this.line.vehicle+"</li>";
                    });
                    $(data).each(function () {
                        toto=toto+this.line.price
                    });
                    $("#jine span").html(toto);
                    $("#show").html(result);
                },"json");
        ...
        $(function () {
            $("#button").click(function () {
                if ($(this).val()=="提交"){
                    ...
                    document.getElementById('txtLeaveDate').setAttribute('disabled', 'disabled')
                    //  为指定的element添加指定的属性，使用原生JS方法
                    jQuery('#txtLeaveDate').attr('disabled', 'disabled')
                    $(this).val("确认订单");
                    if ($(this).val()=="确认订单"){
                        $("#button").click(function () {
                            document.getElementById("p1").innerHTML="<img src=\"images/n11.jpg\" width=\"205\" height=\"62\" />";
                            ...
                            if ($(this).val()=="pay"){
                                $("#button").click(function () {
                                    ...
                                    var time=$("#txtLeaveDate").val();
                                    location.href="/OrderServlet?action=inserts&carID=<%=request.getSession().getAttribute("carIDS")%>&name="+name+"&tel="+tel+"&iden="+iden+"&time="+time+"";
                                });
                            }
                        });
                    }
...
<div class="dingdan-mm">
  ...
  <ul>
  <li><span>*</span> 真实姓名 
      <input name="name" type="text" id="textfield1" size="20" />
      <a href="#">修改</a></li>
        <li><span>*</span> 手机 
      <input name="tel" type="text" id="textfield2" size="20" />
      <a href="#">修改</a></li>
        <li><span>*</span> 身 份 证 
          <input name="iden" type="text" id="textfield3" size="20" />
      <a href="#">修改</a></li>
      <li class="opli"></li>
    </ul>
  </div>
    ...
    <div class="people" align="right" id="jine">
        <strong >应付总额：</strong><span id="to"></span><strong>元</strong>
    </div>
<div class="xyb" id="rf">
